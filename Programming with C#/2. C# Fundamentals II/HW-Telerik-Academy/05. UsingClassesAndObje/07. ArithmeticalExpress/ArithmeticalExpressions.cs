...
namespace _07.ArithmeticalExpress
{
    using System;
    ...
    class ArithmeticalExpressions
    {
        ...
        static void Main()
        {
            Console.Title = "Problem 7.* Arithmetical expressions";
        ...
        public static string Tokenize(string expression)
        {
            ...
            for (int i = 0; i < expression.Length; i++)
            {
                ...
                else if (expression[i] == ')' || expression[i] == ',')
                {
                    ...
                    if (st.Count > 0 && !comma && (st.Peek() == 'p' || st.Peek() == 's' || st.Peek() == 'l'))
                ...
                else
                {
                    ...
                    else
                    {
                        if (resh.Count >= 1)
                        {
                            ...
                            if (!"slnct".Contains(result[i]) && resh.Count > 0)
                                on1 = resh.Pop();

                            if (result[i] == '+')
                                resh.Push(on1 + on2); // x + y
                            else if (result[i] == '-')
                                resh.Push(on1 - on2); // x - y
                            else if (result[i] == '*')
                                resh.Push(on1 * on2); // x * y
                                ...
                                resh.Push(on1 / on2); // x / y
                            else if (result[i] == '^' || result[i] == 'p')
                                resh.Push(Math.Pow(on1, on2)); // pow(x, y) or x ^ y
                            else if (result[i] == 's')
                                resh.Push(Math.Sqrt(on2)); // Sqrt(x)
                            else if (result[i] == 'l')
                                resh.Push(Math.Log(on2, Math.E)); // ln(x)
                            else if (result[i] == 'n')
                                resh.Push(Math.Sin(on2)); // Sin(x)
                            else if (result[i] == 'c')
                                resh.Push(Math.Cos(on2)); // Cos(x)
                            else if (result[i] == 't')
                                resh.Push(Math.Tan(on2)); // Tg(x)
                            else
                            {
                                resh.Push(on1);
                                resh.Push(on2);
