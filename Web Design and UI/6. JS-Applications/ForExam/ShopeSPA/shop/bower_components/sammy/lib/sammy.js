...
(function(factory) {
  ...
  var Sammy,
    ...
    _escapeHTML = function(s) {
      return String(s).replace(/&(?!\w+;)/g, '&amp;').replace(/</g, '&lt;').replace(
        />/g, '&gt;').replace(/"/g, '&quot;');
    },
    _routeWrapper = function(verb) {
      return function() {
        return this.route.apply(this, [verb].concat(Array.prototype.slice.call(
          arguments)));
      };
    },
    _template_cache = {},
  ...
  if (typeof window.console != 'undefined') {
    if (typeof window.console.log === 'function' && _isFunction(window.console
        .log.apply)) {
  ...
  Sammy.targetIsThisWindow = function targetIsThisWindow(event, tagName) {
    ...
    if (!targetWindow || targetWindow === window.name || targetWindow ===
      '_self') {
      return true;
    }
  ...
  $.extend(Sammy.DefaultLocationProxy.prototype, {
    ...
    bind: function() {
      ...
      $(window).bind('hashchange.' + this.app.eventNamespace(),
      ...
      if (_has_history && !app.disable_push_state) {
        // bind to popstate
        $(window).bind('popstate.' + this.app.eventNamespace(),
          function(e) {
            app.trigger('location-changed');
          });
        // bind to link clicks that have routes
        $(document).delegate('a', 'click.history-' + this.app.eventNamespace(),
          function(e) {
    ...
    setLocation: function(new_location) {
      ...
      if (new_location != this.getLocation()) {
        ...
        if (_has_history && !this.app.disable_push_state && /^\//.test(
            new_location)) {
          history.pushState({
            path: new_location
    ...
    _startPolling: function(every) {
      ...
      if (!Sammy.DefaultLocationProxy._interval) {
        ...
        var hashCheck = function() {
          ...
          if (typeof Sammy.DefaultLocationProxy._last_location ==
            'undefined' ||
          ...
          Sammy.DefaultLocationProxy._last_location =
            current_location;
        };
        hashCheck();
  ...
  Sammy.Application = function(app_function) {
    ...
    this.namespace = (new Date()).getTime() + '-' + parseInt(Math.random() *
      1000, 10);
  ...
  Sammy.Application.prototype = $.extend({}, Sammy.Object.prototype, {
    ...
    APP_EVENTS: ['run', 'unload', 'lookup-route', 'run-route',
      'route-found', 'event-context-before', 'event-context-after',
      'changed', 'error', 'check-form-submission', 'redirect',
      'location-changed'
    ],
    ...
    $element: function(selector) {
      return selector ? $(this.element_selector).find(selector) : $(
    ...
    use: function() {
      ...
      try {
        ...
        if (typeof plugin === 'undefined') {
          this.error("Plugin Error: called use() but plugin (" +
            plugin_name.toString() + ") is not defined", e);
        } else if (!_isFunction(plugin)) {
          this.error("Plugin Error: called use() but '" + plugin_name.toString() +
            "' is not a function", e);
        } else {
    ...
    log: function() {
      Sammy.log.apply(Sammy, Array.prototype.concat.apply([this.element_selector],
        arguments));
    },
    ...
    route: function(verb, path) {
      var app = this,
        param_names = [],
        add_route, path_match, callback = Array.prototype.slice.call(
          arguments, 2);
    ...
    run: function(start_url) {
      ...
      // set last location
      this.last_location = null;
      if (!(/\#(.+)/.test(this.getLocation())) && typeof start_url !=
        'undefined') {
      ...
      this.bind('submit', function(e) {
        ...
        var returned = app._checkFormSubmission($(e.target).closest(
          'form'));
    ...
    runRoute: function(verb, path, params, target) {
      ...
      if (route) {
        ...
        if ((path_params = route.path.exec(this.routablePath(path))) !==
          null) {
        ...
        wrapped_route = function() {
          ...
          if (route.callback && route.callback.length) {
            ...
            nextRoute = function() {
              i++;
              if (route.callback[i]) {
                returned = route.callback[i].apply(context,
                  callback_args);
              } else if (app._onComplete && typeof(app._onComplete ===
                  "function")) {
    ...
    contextMatchesOptions: function(context, match_options, positive) {
      ...
      if (_isArray(options.path)) {
        ...
        results = [];
        for (numopt = 0, len = options.path.length; numopt < len; numopt +=
          1) {
          opts = $.extend({}, options, {
      ...
      if (options.only) {
        ...
        return this.contextMatchesOptions(context, options.except,
          false);
      }
      ...
      return positive ? (verb_matched && path_matched) : !(verb_matched &&
        path_matched);
    },
    ...
    _checkLocation: function() {
      ...
      if (!this.last_location || this.last_location[0] != 'get' || this
        .last_location[1] != location) {
        // reset last location
    ...
    _serializeFormParams: function($form) {
      ...
      if (fields.length > 0) {
        ...
        for (i = 1; i < fields.length; i++) {
          queryString = queryString + "&" + this._encodeFormPair(fields[
            i].name, fields[i].value);
        }
      }
      return queryString;
    },
    ...
    _parseFormParams: function($form) {
      ...
      for (i = 0; i < form_fields.length; i++) {
        params = this._parseParamPair(params, form_fields[i].name,
          form_fields[i].value);
      }
      return params;
    },
    ...
    _parseQueryString: function(path) {
      ...
      if (parts && parts[1]) {
        ...
        for (i = 0; i < pairs.length; i++) {
          pair = pairs[i].split('=');
          params = this._parseParamPair(params, _decode(pair[0]),
            _decode(pair[1] || ""));
        }
      }
      return params;
    ...
    _listen: function(name, callback) {
      return this.$element().bind([name, this.eventNamespace()].join(
        '.'), callback);
    },

    _unlisten: function(name, callback) {
      return this.$element().unbind([name, this.eventNamespace()].join(
        '.'), callback);
    }
  ...
  Sammy.RenderContext.prototype = $.extend({}, Sammy.Object.prototype, {
    ...
    then: function(callback) {
      ...
      if (this.waiting) {
        ...
        this.wait();
        window.setTimeout(function() {
          var returned = callback.apply(context, [context.content,
    ...
    load: function(location, options, callback) {
      ...
      return this.then(function() {
        ...
        if (typeof location === 'string') {
          ...
          should_cache = is_json ? options.cache === true : options
            .cache !== false;
          context.next_engine = context.event_context.engineFor(
            location);
          ...
          if (should_cache && (cached = this.event_context.app.templateCache(
              location))) {
            return cached;
          }
          this.wait();
      ...
      return this.load(location).then(function(content) {
        ...
        if (callback) {
          $.each(data, function(i, value) {
            ...
            callback(value, rctx.event_context.interpolate(
              content, idata, engine));
          });
        } else {
          return this.collect(data, function(i, value) {
            ...
            return this.event_context.interpolate(content,
              idata, engine);
          }, true);
        }
      });
    ...
    interpolate: function(data, engine, retain) {
      ...
      return this.then(function(content, prev) {
        ...
        var rendered = context.event_context.interpolate(content,
          data, engine, this.partials);
  ...
  Sammy.EventContext.prototype = $.extend({}, Sammy.Object.prototype, {
    ...
    //
    render: function(location, data, callback, partials) {
      return new Sammy.RenderContext(this).render(location, data,
        callback, partials);
    },
    ...
    //
    renderEach: function(location, name, data, callback) {
      return new Sammy.RenderContext(this).renderEach(location, name,
        data, callback);
    },
    ...
    load: function(location, options, callback) {
      return new Sammy.RenderContext(this).load(location, options,
        callback);
    },
    ...
    toString: function() {
      return "Sammy.EventContext: " + [this.verb, this.path, this.params]
        .join(' ');
    }
